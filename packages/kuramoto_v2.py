...
class Kuramoto:
    ...
    def derivative(self, angles_vec, t, adj_mat, coupling):
        ...
        dxdt = self.natfreqs + coupling * abs(self.natfreqs) * interactions.sum(axis=0)
    ...
    def variance_order_parameter(angles_vec, dt, c):
        # Calculate the first-order derivative of the phase angles
        derivatives = np.gradient(angles_vec, dt, axis=1)
        
        # Calculate the temporal variance of the first-order derivatives
        variances = np.var(derivatives, axis=1)
        
        # Calculate the exponential term, exp(-c * var), for each time step
        exponential_terms = np.exp(-c * variances)
        
        # Calculate the average of the exponential terms over all time steps
        average_exponential = np.mean(exponential_terms)
        
        # Calculate the variance order parameter
        N = len(angles_vec)
        variance_order_parameter = (1/N) * average_exponential
        
        return variance_order_parameter